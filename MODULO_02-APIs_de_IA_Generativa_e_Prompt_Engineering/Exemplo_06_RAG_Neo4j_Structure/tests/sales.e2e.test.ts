...
describe("Sales Analytics Reporter - E2E Tests", async () => {
  ...
  it("Query students who bought specific course - Should provide details", async () => {
    const response = await makeASalesRequest("Who bought Formação JavaScript Expert?")
  ...
  it("Query total revenue from credit cards - Should include analytics", async () => {
    const response = await makeASalesRequest("What is the total revenue from credit card payments?")
  ...
  it("Revenue distribution analysis - Should show percentages and comparisons", async () => {
    const response = await makeASalesRequest("Show me the revenue distribution across all courses")
    ...
    assert.ok(body.answer.match(/\d+%|\$\d+|revenue/i), "Answer should contain analytical metrics")
  })

  it("Query students with 100% progress - Should provide insights", async () => {
    const response = await makeASalesRequest("Which students have 100% progress in their courses?")
  ...
  it("Payment method analysis - Should compare different methods", async () => {
    const response = await makeASalesRequest("What are the most popular payment methods?")
  ...
  it("Edge case: Students with progress but no purchase - Should handle gracefully", async () => {
    const response = await makeASalesRequest("Are there students with progress in a course they never bought?")
  ...
  it("Query students who never started a course - Should show engagement gaps", async () => {
    const response = await makeASalesRequest("Which students bought a course but never started it?")
  ...
  it("Follow-up questions - Should provide relevant suggestions", async () => {
    const response = await makeASalesRequest("Show me the top performing courses")
    ...
    assert.ok(body.followUpQuestions.length >= 2, "Should provide at least 2 follow-up questions")
  })
})
